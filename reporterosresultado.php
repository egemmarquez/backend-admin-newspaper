...
<div class="Estilo1" style="width:100%" id="menu_right">
<div id="margentexto">
...
<?php
 ...
 ?>
 <h3>
NOTAS DEL DIA <?php echo $_POST['FINICIO']; ?> AL DIA  <?php echo $_POST['FFINAL']; ?> POR EL REPORTERO  <?php echo nombrereportero($_POST['REPORTEROS_LISTA']); ?>
</h3>
...
$consulta = 'SELECT * from tabart where art_Fecha BETWEEN \''.strtotime($_POST['FINICIO']).'\' and \''.strtotime($_POST['FFINAL']).'\' and NOT_REPORTERO = '.$_POST['REPORTEROS_LISTA'].'  order by '.$ID.' desc';
$_pagi_sql = ''.$consulta.'';
...
$paginador .= '
<th>'.$row[''.$ID.''].'
<td><a href="../noticia.php?article='.$row[''.$ID.''].'" target="_blank">'.$row[''.$TITULO.''].'</a><br>
<td><center><font size="1">'.regresa_fecha($row[''.$FECHA.'']).'</font>
</tr><tr>
';
}
echo $paginador;
echo "<h3>Total de Notas:";
echo  $_pagi_totalReg;
echo "</h3>";
 ?>
 </table>
